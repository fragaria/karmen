  ...
  return (
    <>
     ...
     {labels.map(function (label, index) {
       return <span className={"printer-status-label " + label.color} data-tip={label.detail}> {label.status}</span>
...
export const PrinterState = ({printer}) => {
  ...
  if (printer.client.octoprint && printer.client.octoprint.error) {
    ...
    if (err.code === "moved-to-background") {
      // if WS API returns 502 or 504, backend considers it as unknown response, but it means pill is offline
      return buildLabels([{color: "orange", status: "Connecting", detail: "We are trying to connect to your device."}])
    }
    ...
    if (err.code === "permission-denied") {
      return buildLabels(labels.concat({
        ...
        detail: "Looks like this printer is protected and you did not entered the correct API key"
      }))
    }
    labels.push({color: "red", status: "Error", detail: err.detail})
  } else {
    labels.push({color: "green", status: "Online", detail: "We are connected."});
  }
  ...
  if (printer.client.octoprint.printer.state.flags) {
    ...
    if (printerStates.error) {
      labels.push({color: "red", status: "Error", detail:printer.client.octoprint.printer.state.text})
    }
