    ...
    for (let printer of result.data.items) {
      if (printers.checkQueue) {
        ...
        if (existing === null || existing === undefined) {
          const poll = ['Printing', 'Paused'].indexOf(printer.status.state) > -1 ? PRINTER_RUNNING_POLL : PRINTER_IDLE_POLL;
          dispatch(setPrinterPollInterval(printer.host, poll));
          queueLoadPrinter(dispatch, getState, printer.host, ['job', 'status', 'webcam'], poll);
        }
      }
    }
    return result;
...
export const queueLoadPrinter = (dispatch, getState, host, fields, delay) => {
  setTimeout(() => {
    const { printers } = getState();
    const previousInfo = printers.printers && printers.printers.find((p) => p.host === host);
    dispatch(loadPrinter(host, fields))
      .then((result) => {
        const { printers } = getState();
        if (printers.checkQueue[host] > 0) {
          let interval = printers.checkQueue[host];
          // detect if state change and we need to adjust interval        
          if (previousInfo && previousInfo.status && result && result.data && result.data.status && result.data.status.state !== previousInfo.status.state) {
            interval = ['Printing', 'Paused'].indexOf(result.data.status.state) > -1 ? PRINTER_RUNNING_POLL : PRINTER_IDLE_POLL;
            dispatch(setPrinterPollInterval(host, interval));
          }
          // enqueue next check if result is ok
          if (result.status === 200) {
            queueLoadPrinter(dispatch, getState, host, ['job', 'status', 'webcam'], interval);
          } else {
