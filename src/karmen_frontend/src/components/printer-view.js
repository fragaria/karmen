...
export const PrinterActions = ({ host, onPrinterDelete }) => {
  return (
    <div className="box-actions">
      ...
      <Link to={`/printers/${host}`}>
        <i className="icon icon-cog"></i>
      </Link>
      <button className="plain" onClick={onPrinterDelete}>
    ...
    <div className="tags">
      {printer.client.access_level === "protected" ? (
        ...
        <>
          <span
            className={`tag ${
              printer.client.connected ? "state-active" : "state-inactive"
            }`}
          >
          ...
          <span className={`tag ${printerStatusClass}`}>
...
export const PrinterTemperatures = ({ temperature }) => {
  return (
    <>
      {temperature.tool0 && (
        <>
          <Temperature name="Tool" {...temperature.tool0} />,
        </>
      )}
  ...
  return (
    <div className="printer-state">
      ...
      {printer.status.temperature ? (
        <PrinterTemperatures temperature={printer.status.temperature} />
      ) : (
        <>&nbsp;</>
      )}
      {printer.job && printer.job.name && (
        <p>
          Printing:{" "}
          <strong>{(printer.job && printer.job.name) || "\u00A0"}</strong>
        </p>
      )}
      {props &&
        (props.filament_type ||
          ...
          <p>
            Setup: {props.filament_type}{" "}
            {props.filament_color && <>({props.filament_color})</>}
    ...
    if (showDeleteModal) {
      return (
        ...
        >
          <h1>Are you sure?</h1>
          <p>
            You can add the printer back later by simply adding{" "}
            <code>{printer.host}</code> again.
          </p>
          <button
            type="submit"
            onClick={() => {
      ...
      return (
        ...
        >
          ...
          <button
            type="submit"
            onClick={() => {
              changeCurrentJobState(printer.host, "cancel").then(() => {
                this.setState({
                  showDeleteModal: false,
                  showCancelModal: false
                });
