  ...
  render() {
    ...
    if (showSuspendRow) {
      return (
        <tr className="inverse">
          <td colSpan="4">
            Do you really want to {user.suspended ? 'allow' : 'disallow'} <strong>{user.username}</strong>?
          </td>
          <td className="action-cell">
            ...
            <button className="plain" title="Confirm" onClick={() => {
              onUserChange(user.uuid, user.role, !user.suspended)
                .then(() => {
                  this.setState({
                    showSuspendRow: false
                  });
    ...
    if (showChangeRoleRow) {
      return (
        <tr className="inverse">
          <td colSpan="4">
            Do you really want to {user.role === "admin" ? 'demote' : 'promote'} <strong>{user.username}</strong> to {user.role === "admin" ? 'user' : 'admin'}?
          </td>
          <td className="action-cell">
            <button className="plain" title="Cancel" onClick={() => {
              this.setState({
            ...
            <button className="plain" title="Confirm" onClick={() => {
              ...
              onUserChange(user.uuid, newRole, user.suspended)
                .then(() => {
                  this.setState({
                    showChangeRoleRow: false
                  });
      ...
      <tr>
        ...
        <td className="action-cell">
          {currentUuid !== user.uuid && <>
            ...
            <button className="plain icon-link" title="Change role" onClick={() => {
              this.setState({
                showChangeRoleRow: true
              })
            }}><i className="icon icon-file-text2"></i></button>
          </>}
        </td>
      </tr>
    );
...
class UsersTable extends React.Component {
  ...
  componentDidMount() {
    ...
    if (!userList.pages || !userList.pages.length || !usersLoaded) {
      this.reloadTableWith(null, userList.orderBy, userList.usernameFilter, userList.limit);
    }
  }
    ...
    if (prevState.currentPageIndex !== this.state.currentPageIndex) {
      ...
      if (prevState.currentPageIndex < this.state.currentPageIndex) {
        ...
        if (!userList.pages.find((p) => p.startWith === nextStartWith)) {
          this.reloadTableWith(nextStartWith, userList.orderBy, userList.usernameFilter, userList.limit, this.state.currentPageIndex);
        }
      }
    }
  }
  ...
  reloadTableWith(nextStart, orderBy, usernameFilter, limit, currentPageIndex) {
    ...
    loadUsersPage(nextStart, orderBy, usernameFilter, limit)
      .then(() => {
        this.setState({
    ...
    const sortFactory = (column) => {
      return () => {
        ...
        if (userList.orderBy === `+${column}`) {
          order = `-${column}`;
        } else if (userList.orderBy === `-${column}` && userList.orderBy !== '+username') {
          order = '+username';
        }
        this.reloadTableWith(null, order, userList.usernameFilter, userList.limit);
      }
    }
      ...
      <div>
        <form className="inline-form">
          ...
          <button type="submit" onClick={(e) => {
            ...
            const { userList } = this.props;
            this.reloadTableWith(null, userList.orderBy, willBeFilter, userList.limit);
          }}>Filter</button>
            ...
            <>
              <table>
                <thead>
                  <tr>
                    <th>
                      <button className={`plain sorting-button ${userList.orderBy.indexOf('uuid') > -1 ? 'active' : ''}`} onClick={sortFactory("uuid")}>UUID</button>
                    </th>
                    <th>
                      <button className={`plain sorting-button ${userList.orderBy.indexOf('username') > -1 ? 'active' : ''}`} onClick={sortFactory("username")}>Username</button>
                    </th>
                    <th>
                      <button className={`plain sorting-button ${userList.orderBy.indexOf('role') > -1 ? 'active' : ''}`} onClick={sortFactory("role")}>Role</button>
                    </th>
                    <th>Allowed?</th>
                    <th>Actions</th>
                  </tr>
