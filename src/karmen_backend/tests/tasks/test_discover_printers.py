...
class DiscoverPrintersTest(unittest.TestCase):
    ...
    @mock.patch('server.database.get_network_devices', return_value=[
        {"mac": "06:43:ac:11:00:02", "ip": "172.17.0.2", "client_props": {"connected": True}, "retry_after": datetime.utcnow() + timedelta(hours=1), "disabled": False},
        {"mac": "34:97:f6:3f:f1:96", "ip": "192.168.1.1", "client_props": {"connected": True}, "retry_after": datetime.utcnow() + timedelta(hours=-1), "disabled": False},
    ])
    @mock.patch('server.tasks.discover_printers.get_avahi_hostname', return_value='router.asus.com')
    @mock.patch('server.tasks.discover_printers.update_printer')
    @mock.patch('server.tasks.discover_printers.sniff_printer.delay')
    def test_skip_device_mac(self, mock_delay, mock_update_printer, mock_avahi, \
        mock_db_devices, mock_get_printers, mock_get_devices):
        discover_printers()
    ...
    @mock.patch('server.database.get_network_devices', return_value=[
        {"mac": "06:43:ac:11:00:03", "ip": "172.17.0.2", "client_props": {"connected": True}, "retry_after": datetime.utcnow() + timedelta(hours=1), "disabled": False},
        {"mac": "34:97:f6:3f:f1:97", "ip": "192.168.1.1", "client_props": {"connected": True}, "retry_after": datetime.utcnow() + timedelta(hours=-1), "disabled": False},
    ])
    @mock.patch('server.tasks.discover_printers.get_avahi_hostname', return_value='router.asus.com')
    @mock.patch('server.tasks.discover_printers.update_printer')
    @mock.patch('server.tasks.discover_printers.sniff_printer.delay')
