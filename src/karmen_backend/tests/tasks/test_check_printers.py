...
class CheckPrintersTest(unittest.TestCase):
    ...
    def test_activate_responding_printer(self, mock_redis, mock_get_data, mock_update_printer, mock_get_printers):
        class Response():
        ...
        def mock_call(ip, **kwargs):
            if ip == '5678' and "endpoint" in kwargs and kwargs["endpoint"] == "/api/settings":
        ...
        mock_redis.set.assert_has_calls([mock.call('webcam_5678', 'http://5678/webcam/?action=stream')])
    ...
    def test_no_fail_on_broken_redis(self, mock_logger, mock_redis, mock_get_data, mock_update_printer, mock_get_printers):
        class Response():
            def __init__(self, status_code, contents, json=None):
                self.status_code = status_code
                self.contents = contents
