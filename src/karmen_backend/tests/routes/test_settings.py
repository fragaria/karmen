...
class SettingsRoute(unittest.TestCase):
    def test_get(self):
        with app.test_client() as c:
            ...
            self.assertEqual(len(response.json), len(app.config["CONFIGURABLE_SETTINGS"]))

    def test_set_bad_data(self):
        with app.test_client() as c:
            response = c.post('/settings', json=[
                {"some": "thing"}
            ])
    ...
    def test_set_data(self):
        with app.test_client() as c:
            ...
            response = c.post('/settings', json=[
                {"key": "network_discovery", "val": not orig}
            ])
